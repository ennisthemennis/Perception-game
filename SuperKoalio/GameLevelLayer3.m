...
#import "Level3DieLayer.h"


@interface GameLevelLayer3()
{
    CCTMXTiledMap *map;
    Player * player;
    CCTMXLayer *walls;
...
@end
@implementation GameLevelLayer3
...
+(CCScene *) scene
{
	...
	GameLevelLayer3 *layer = [GameLevelLayer3   node];
...
-(id) init
{
	...
	if( (self=[super init]) ) {
        ...
        map = [[CCTMXTiledMap alloc] initWithTMXFile:@"level3.tmx"];
        ...
        alien4.scale = 0.2;
        alien4.position = ccp(3824, 50);
        shake = [CCRepeatForever actionWithAction: [CCAnimate actionWithAnimation:shaking restoreOriginalFrame:NO]];
        [alien4 runAction:shake];
        [map addChild:alien4 z:15];
        alien5 = [CCSprite spriteWithSpriteFrameName:@"alien0.png" ];
        alien5.scale = 0.2;
        alien5.position = ccp(5768, 50);
        shake = [CCRepeatForever actionWithAction: [CCAnimate actionWithAnimation:shaking restoreOriginalFrame:NO]];
        [alien5 runAction:shake];
        [map addChild:alien5 z:15];
        instructions = [CCSprite spriteWithFile:@"rounded-blue-sq-3.png" ];
...
-(void)checkForStuck
{
    if(player.position.x>4880 && player.enemiesKilled < 3)
    {
        if (!stuck)
        {
...
-(void)gameOver {
	...
	if (won) {
        ...
        [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"level3complete"];
		[[CCDirector sharedDirector] replaceScene:[Level3DoneLayer scene]];
	} else {
		[[CCDirector sharedDirector] replaceScene:[Level3DieLayer scene]];
    }
}
-(void)checkForWin {
    if (player.position.x > 6300.0) {
        won = TRUE;
        [self gameOver];
    }
}
...
-(void) moveAliens
{
    ...
    if (alien5here)
    {
        alien5.position = ccpAdd(alien5.position, velocity5);
    }
}
-(void) deathCheck
{
    ...
    if(alien5here)
    {
        if (CGRectIntersectsRect(bullet.boundingBox, alien5.boundingBox))
        {
            [alien5 removeFromParentAndCleanup:YES];
    ...
    if (alien5here)
    {
        ...
        if (CGRectIntersectsRect(player.boundingBox, alien5.boundingBox))
        {
